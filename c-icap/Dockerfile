...
ADD ./testscript.sh /usr/local/c-icap/testscript.sh
ADD ./input /usr/local/c-icap/input
ADD ./output /usr/local/c-icap/output

RUN chmod +x /usr/local
CMD ["bash", "/usr/local/c-icap/testscript.sh"]
